...
def gendata(T, dt, noise=2, plot=True, output=True):
    ...
    Ma = np.array(Mi)
    normM = np.real(Ma) / (np.amax(np.real(Ma)) - np.amin(np.real(Ma)))
    M = normM - np.amin(normM)
...
def getcorrelation(datafile, Detector, N, a, b, dt, strength=1, plot1=False, plot2=False, showrs=False, printoutput=False, histogram=False, crit=False, r=[], error=[] ,ostrengths=[], grms=[] ,folderpath=''):
    ...
    for i in curves:
        ...
        if plot2 == True:
            ...
            plt.xlabel('Time(s)')
            plt.ylabel('Average Absolute Flux(V)')
    ...
    if histogram == True:
        plt.figure()
        t = plt.hist(rs, 10, )
        binwidth=t[1][1]-t[1][0]
        scale=len(rs)*binwidth
        yRN=np.array(yR)*scale
        if crit==True:
            plt.title('Distribution of Correlation Coefficients at Critical Strength: '+str(strength))
            plt.xlabel('Correlation Coefficient (R)')
            plt.ylabel('Number of Occurrences')
            plt.plot(Rrange, yRN)
            plt.savefig(f'{folderpath}/Distribution of Correlation Coefficients at Critical Strength:{str(strength)}.png')
        else:
            plt.title('Distribution of Correlation Coefficients at Strength: ' + str(strength))
            plt.xlabel('Correlation Coefficient (R)')
            plt.ylabel('Number of Occurrences')
            plt.plot(Rrange, yRN)
            plt.savefig(f'{folderpath}/Distribution of Correlation Coefficients at Strength:{str(strength)}.png')
            #plt.show()
        ...
        with Manager() as manager:
            ...
            for strengths in tqdm(splitstrengths):
                ...
                for i in tqdm(strengths):
                    arguments.append([datafile, Detector, trials, A, B, dt, i, False, False, False, False, histogram, False, r, error, ostrengths, grms, folderpath])
            ...
            if plot1==True:
                ...
                plt.savefig(f'{folderpath}/Strength of Correlation at Different Strengths of Injected Signal')
            ...
            if plot2==True:
                ...
                plt.legend()
                plt.savefig(f'{folderpath}/R To Error Ratio')
            ...
            if savedata==True:
                result='\n\n------------------------------------------------------------------------------------------\n\n'+'ECM Run Identification Number: '+str(ECMRun)+', Data File: '+datafile+', Detector Number: '+str(Detector)+', Duration of ECM Data Used: '+str(B-A)+'s'+', Time Resolution: '+str(dt)+'s'+', Number of Sampled Strengths: '+str(Nstrengths)+', Number of Rs per Gaussian Fit: '+str(trials)+', Direct Observation Weather Grade: '+str(obgrade)+', Weather Grade from Plotted Data: '+str(plgrade)+', Night Sky Background (NSB): '+str(NSB)[0]+str(NSB)[1]+str(NSB)[2]+'('+str(NSB)[3]+')'+'\n\n'+'---->Critical Strength in RMS: Lower Bound:'+str(critslow)+'; Estimate: '+str(crits)+'; Upper Bound: '+str(critshigh)+'\nCritical Strength in Volts: Lower Bound:'+str(critslowv)+'; Estimate:'+str(critsv)+'; Upper Bound:'+str(critshighv)+'\nCritical R: Lower Bound: '+str(critrlow)+'; Estimate: '+str(critr)+'; Upper Bound: '+str(critrhigh)
...
parser=argparse.ArgumentParser(description='find 5 Sigma significance threshold for XRB detection from ECM data')
parser.add_argument('-dp', '--datapath', type=str, help='path to ECM data file')
parser.add_argument('-op', '--outputpath', type=str, help='path to where output results are stored')
parser.add_argument('-er', '--ecmrun', type=str, help='which run of ECM data is being used')
parser.add_argument('-tn', '--telescopenumber', type=int, help='which telescope is being used')
parser.add_argument('-ss', '--samplestrengths', type=int, help='how many sample strengths (magnification of sample signals) are used for the fit')
parser.add_argument('-nc', '--coresavailable', type=int, help='how many cores are available for the job')
parser.add_argument('-tr', '--numbertrials', type=int, help='number of trials conducted per sample strength')
